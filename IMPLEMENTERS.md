...
**Proof-only (the §11 v1.4 kind) is optional to implement, but you must not mishandle it.** A core reader that refuses a proof-only record loudly (as an unknown `kind`) is correct — that is read-old-forever behaviour, not a bug. A complete v1.4 reader implements §11: the closed-enum kind classifier (unknown kind refuses; kind-absent-with-valid-parts is a legacy git-bundle), the storage cross-check that keeps existence and permanence from blurring, reconstruction refusal (a proof-only publish is never reconstructed — you return its fingerprint and never a file, not even a partial one), the ancestor recovery walk, and VERIFY (compare a caller-supplied local bundle against the on-chain commitment; never say "verified" off a fingerprint checked against no bytes). The one law is that existence and permanence must never blur — a proof-only publish can never be read, reported, or verified as bytes-on-chain. The nineteen §11 vectors pin all of it.

**Test vectors** are in [`bgit-vectors.test.mjs`](bgit-vectors.test.mjs). Forty-nine of them — thirty for the core format plus nineteen for the §11 proof-only kind — covering every rejection class, fork races at every height, claim replay, malformed input, and the proof-only seam (closed-enum classifier, storage cross-check, reconstruction refusal, ancestor recovery, VERIFY). You do not need to use our test harness — the vectors themselves are the value, and they encode the rules that are easy to get wrong. Read them by provenance, per the hierarchy above: the rejection vectors encode spec rules and are independently checkable against [SPEC.md](SPEC.md); the positive signature round-trips only prove agreement with our implementation; the end-to-end vector (publisher to reader to byte-identical bundle to `git clone`) is the one that anchors to Monero's own tooling. Passing all forty-nine is necessary and not sufficient — the clone is what counts.
...
>
> **Reconstructing those three is the happy path and it is not sufficient.** Your implementation is not finished until it also demonstrably handles: dust-to-repo behaviour across every record type; JSON and size constraints enforced BEFORE any parse; authorization checked BEFORE fork comparison; two competing valid records in the SAME BLOCK, which must resolve by intra-block order or refuse with a typed ambiguity error rather than guessing; unknown record versions and types coexisting with known ones without influencing winner selection; and complete-or-refuse output that is atomic, leaving no partial artifact behind on failure. Most of the test vectors are rejections for this reason — the failure cases are the specification. (The §11 proof-only kind is a separately-specified v1.4 extension; a v1 core reader may refuse proof-only records loudly as an unknown kind, which is correct.)
>
...
We will not pay you, and we are not going to pretend this is a bounty. This is an open format under MIT with a published spec and forty-nine test vectors, and the invitation is exactly what it looks like: we think the thing is worth checking, we cannot check it ourselves in the way that counts, and we would rather say so than let a single implementation quietly become the specification.

A second reader does not make bgit a format — it already is one, open, specified, and running. What a second reader makes is a format nobody has to take our word for.

*(That last line is a slogan, not a guarantee. Two agreeing implementations are better evidence than one, and they do not by themselves establish permanence or censorship resistance — those rest on the data being mined and on someone choosing to keep a copy. The honest limits section of the [README](README.md) still stands unchanged.)*
